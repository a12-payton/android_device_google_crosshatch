...
static std::string getVendorFunctions() {
    if (GetProperty(BUILD_TYPE, "") == "user")
        return "user";

    std::string bootMode = GetProperty(PERSISTENT_BOOT_MODE, "");
    std::string persistVendorFunctions = GetProperty(PERSISTENT_VENDOR_CONFIG, "");
...
Status UsbGadget::setupFunctions(long functions,
    ...
    if (callback) {
        ...
        ScopedAStatus ret = callback->setCurrentUsbFunctionsCb(
            functions, gadgetPullup ? Status::SUCCESS : Status::ERROR, in_transactionId);
        if (!ret.isOk()) {
